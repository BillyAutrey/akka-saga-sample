...
object TypedSagaActor {
    ...
    case WrappedInventoryResponse(msg) =>
      msg match {
        case InventoryActor.ReservationMade(order) =>
          ...
          state.payment ! PaymentActor.ProcessPayment(PaymentActor.Amount(1,99),order.orderId, state.paymentResponseWrapper)
  ...
  def waitingOnInventoryCompensatingAction(state: TypeState, context: ActorContext[_]): Behavior[SagaCommand] = Behaviors.receiveMessage{
  ...
  def waitingOnPaymentCompensatingAction(state: TypeState, context: ActorContext[_]): Behavior[SagaCommand] = Behaviors.receiveMessage{
    case WrappedPaymentResponse(response) =>
      response match {
        ...
        case PaymentActor.RefundSucceeded(amount, orderId) =>
          state.inventory ! InventoryActor.UnreserveItem(state.order.getOrElse(Order.empty.copy(orderId = orderId)), state.inventoryResponseMapper)
          waitingOnInventoryCompensatingAction(state, context)
        case paymentMsg =>
